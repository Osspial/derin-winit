...
use winapi::um::{winuser, winbase, shellapi, processthreadsapi, commctrl};
...
pub struct EventLoop<T> {
    ...
    thread_id: DWORD,
    pub(crate) event_send: Sender<Event<T>>,
    event_recv: Receiver<Event<T>>
}
...
impl<T> EventLoop<T> {
    ...
    pub fn with_dpi_awareness(dpi_aware: bool) -> EventLoop<T> {
        ...
        EventLoop { thread_id, event_send, event_recv }
    }
        ...
        unsafe {
            ...
            'main: loop {
                ...
                while has_message {
                    ...
                    while let Ok(event) = event_loop.events_loop.event_recv.try_recv() {
    ...
    pub fn send_event(&self, event: T) -> Result<(), EventLoopClosed> {
        unsafe {
            if winuser::PostThreadMessageA(self.thread_id, *WAKEUP_MSG_ID, 0, 0) != 0 {
                self.event_send.send(Event::UserEvent(event)).ok();
                Ok(())
            } else {
...
pub unsafe extern "system" fn callback<T>(
    window: HWND,
    msg: UINT,
    wparam: WPARAM,
    lparam: LPARAM,
