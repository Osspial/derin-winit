...
pub fn init_wayland(window: &winit::window::Window) {
    #[cfg(any(target_os = "linux", target_os = "dragonfly", target_os = "freebsd", target_os = "netbsd", target_os = "openbsd"))]
    {
        if let Some(winit_display) = window.get_wayland_display() {
            if let Some(surface) = window.get_wayland_surface() {
                ...
                use self::sctk::reexports::client::protocol::wl_surface::WlSurface;
                ...
                let surface: Proxy<WlSurface> = unsafe { Proxy::from_c_ptr(surface as *mut _) };
